...
if ( __name__ == "__main__" ):
    ...
    for directory in sys.argv[1:]:
        ...
        if ( not os.path.isfile( addon_xml )):
            print "{0}: Couldn't find 'addon.xml'".format(directory)
            continue
        icon =  os.path.join( directory, "icon.png" )
        if ( not os.path.isfile( icon )):
            print "{0}: Couldn't find 'icon.png'".format(directory)
            continue
        try:
            tree = etree.parse(addon_xml)
        except:
            print "{0}: Could not parse addon.xml".format(directory)
            continue
        ...
        if not version:
            print "{0}: does not contain <version> element".format(addon_xml)
            continue
        id = root.get('id')
        if not id:
            print "{0}: does not contain <id> element".format(addon_xml)
            continue
        print "######################################"
        print "Addon '{0}', version '{1}'.".format(id, version)
        print "######################################"
        ...
        zipfolder = os.path.join(repofolder,id)
